...
forces_beta_exp = cp.ElementwiseKernel(
    'raw float64 S, raw float64 I, float64 beta, float64 gamma, float64 D, float64 alpha, float64 a, uint32 Lx, uint32 Ly',
    'float64 fS, float64 fI',
    '''
    int x = i % Lx;
    int y = (int) i/Lx;

    fS = -beta*S[i]*powf(I[i],alpha)*exp(-a/I[i]);
    fI = beta*S[i]*powf(I[i],alpha)*exp(-a/I[i]) - gamma*I[i] + D*(I[(x+1)%Lx + Lx*y] + I[(x-1+Lx)%Lx + Lx*y] + I[x + Lx*((y+1)%Ly)] + I[x + Lx*((y-1+Ly)%Ly)] - 4*I[i]);
    ''',
    'forces_beta_exp')
...
class System:
    ...
    def set_tilded_initial_conditions(self,m = 0, I0 = 1):
    ...
    def update_exp(self):
        forces_beta_exp(self.S,self.I,self.beta,self.gamma,self.D,self.alpha,self.a,self.Lx,self.Ly,self.fS,self.fI)
        euler(self.fS,self.fI,self.dt,self.S,self.I)
        self.t += self.dt
        self.t_it += 1
    ...
    def update2_exp(self):
        ...
        forces_beta_exp(self.S[:,x1:x2],self.I[:,x1:x2],self.beta[:,x1:x2],self.gamma,self.D,self.alpha,self.a,x2-x1,self.Ly,self.fS[:,x1:x2],self.fI[:,x1:x2])
        self.fS[:,x1] = self.fS[:,x2-1] = self.fI[:,x1] = self.fI[:,x2-1] = 0
        euler(self.fS[:,x1:x2],self.fI[:,x1:x2],self.dt,self.S[:,x1:x2],self.I[:,x1:x2])
        self.t += self.dt
        self.t_it += 1
    ...
    def roll_rows(self,arr, roll_amounts):
        return cp.array([cp.roll(arr[i,:],roll_amounts[i]) for i in range(arr.shape[0])])

    #def f(self):
    #    return cp.mean(self.roll_rows(self.I,-self.u()+self.Lx//2),axis=0)
    #
    #def f_std(self):
    #    return cp.std(self.roll_rows(self.I,-self.u()+self.Lx//2),axis=0)

    #def g(self):
    #    return cp.mean(self.roll_rows(self.S,-self.u()+self.Lx//2),axis=0)
    #
    #def g_std(self):
        return cp.std(self.roll_rows(self.S,-self.u()+self.Lx//2),axis=0)
